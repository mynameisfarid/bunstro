// @bun
import {
DatabaseManager_default,
Logger,
Logger_default,
__commonJS,
__toESM
} from "./chunk-406d26fb795b694f.js";
...
var require_mime_types = __commonJS((exports) => {
  ...
  var extname2 = import.meta.require("path").extname;
...
var ParseErr = function(message, str, indx) {
...
var RuntimeErr = function(originalError, str, lineNo, path$1) {
...
var resolvePath = function(templatePath, options) {
...
var dirIsChild = function(parent, dir) {
...
var compileToString = function(str, options) {
  const config = this.config;
...
var trimWS = function(str, config, wsLeft, wsRight) {
  let leftTrim;
  let rightTrim;
...
var getLineNo = function(str, index) {
    ...
    while (closeTag = parseCloseReg.exec(str))
      if (closeTag[1]) {
        ...
        if (char === "/*") {
          ...
          if (singleQuoteReg.exec(str))
            parseCloseReg.lastIndex = singleQuoteReg.lastIndex;
          else
            ParseErr("unclosed string", str, closeTag.index);
        } else if (char === "\"") {
          doubleQuoteReg.lastIndex = closeTag.index;
          if (doubleQuoteReg.exec(str))
            parseCloseReg.lastIndex = doubleQuoteReg.lastIndex;
          else
            ParseErr("unclosed string", str, closeTag.index);
        } else if (char === "`") {
          templateLitReg.lastIndex = closeTag.index;
          if (templateLitReg.exec(str))
...
var handleCache = function(template, options) {
...
var render = function(template, data, meta) {
  let templateFn;
  const options = {
    ...meta,
    async: false
...
var renderAsync = function(template, data, meta) {
  let templateFn;
  const options = {
    ...meta,
    async: true
...
var renderString = function(template, data) {
  const templateFn = this.compile(template, { async: false });
  return render.call(this, templateFn, data);
};
var renderStringAsync = function(template, data) {
...
var Eta$1 = class {
  ...
  loadTemplate(name, template, options) {
    if (typeof template === "string")
      ((options?.async) ? this.templatesAsync : this.templatesSync).define(name, this.compile(template, options));
    else {
...
class ViewEngine {
  ...
  register(name, path2) {
    ...
    Logger_default.debug(`View engine registered: ${name} -> ${path2}`);
  }
  setDefault(path2) {
    this.defaultPath = path2;
    this.register("default", path2);
  }
  getEngine(name) {
    const engineName = name || "default";
    if (!this.engines.has(engineName)) {
      const path2 = name ? `applications/${name}/views` : this.defaultPath;
      if (existsSync(path2)) {
        this.register(engineName, path2);
      } else {
        throw new Error(`View engine '${engineName}' not registered and path '${path2}' not found`);
      }
    }
  ...
  async render(template, data = {}, app) {
    if (!app) {
    ...
    try {
      ...
      Logger_default.error(`View render error: ${error.message}`, {
        viewsPath,
        template,
        app,
        error: error.stack
  ...
  async response(template, data = {}, options = {}) {
  ...
  async exists(template, app) {
    try {
      const { viewsPath } = this.getEngine(app);
      const templatePath = join2(viewsPath, `${template}.vto`);
...
var view = ViewEngine.getInstance();
var ViewEngine_default = view;
...
class HttpServer {
  ...
  async start() {
    ...
    Logger_default.log("app", `\uD83D\uDE80 Server running on http://${this.config.host}:${this.config.port}`);
  }
  async handleRequest(req) {
    ...
    try {
      if (this.config.staticPath && path2.startsWith(this.config.staticUrl || "/")) {
      ...
      Logger_default.error(`Request error: ${error.message}`);
      return this.config.onError ? this.config.onError(error, req) : new Response(JSON.stringify({ error: "Internal Server Error" }), {
        status: 500,
        headers: JSON_HEADERS
      });
    }
  }
  async serveStatic(path2) {
    if (!this.config.staticPath || !this.config.staticUrl)
      return null;
    if (!path2.startsWith(this.config.staticUrl))
      return null;
    if (process.env.APP_ENV === "production" && this.staticCache.has(path2)) {
      return this.staticCache.get(path2);
    }
    const relativePath = path2.slice(this.config.staticUrl.length);
    const filePath = join3(this.config.staticPath, relativePath);
    try {
      ...
      if (process.env.APP_ENV === "production") {
        this.staticCache.set(path2, response);
      }
      return response;
    } catch {
      return null;
  ...
  buildContextFast(req, url, params, query) {
    return {
      request: req,
      method: req.method,
      url,
      ...
      db: DatabaseManager_default.createContextFacade(),
      view: ViewEngine_default,
      ...
      html(template, data, status) {
        if (status) {
          return ViewEngine_default.response(template, data, { status });
        }
        return ViewEngine_default.response(template, data);
      },
  ...
  handleError(error) {
    Logger_default.error("Server error:", error);
  ...
  async stop() {
    Logger_default.info("Server stopped");
    if (this.server) {
      this.server.stop();
      Logger_default.info("Server stopped");
    }
...
\uD83D\uDED1 Shutting down (${signal}) gracefully`);
    try {
      await DatabaseManager_default.shutdown();
      if (this.server) {
        this.server.stop();
        Logger_default.info("Server stopped");
      }
...
class Router {
  ...
  get(path2, handler, options) {
    return this.addRoute("GET", path2, handler, options);
  }
  post(path2, handler, options) {
    return this.addRoute("POST", path2, handler, options);
  }
  put(path2, handler, options) {
    return this.addRoute("PUT", path2, handler, options);
  }
  patch(path2, handler, options) {
    return this.addRoute("PATCH", path2, handler, options);
  }
  delete(path2, handler, options) {
    return this.addRoute("DELETE", path2, handler, options);
  }
  resource(path2, controller) {
    if (controller.index)
      this.get(path2, controller.index.bind(controller));
    if (controller.store)
      this.post(path2, controller.store.bind(controller));
    if (controller.show)
      this.get(`${path2}/:id`, controller.show.bind(controller));
    if (controller.update)
      this.put(`${path2}/:id`, controller.update.bind(controller));
    if (controller.destroy)
      this.delete(`${path2}/:id`, controller.destroy.bind(controller));
    return this;
  }
  ...
  async loadRoutes(filePath) {
    try {
      ...
      throw new Error(`Failed to load routes from ${filePath}: ${error.message} ${error}`);
    }
  }
  addRoute(method, path2, handler, options) {
    const fullPath = this.prefix + path2;
  ...
  pathToRegex(path2) {
    const keys = [];
    const pattern = path2.replace(/\/:([^/]+)/g, (_, key) => {
      keys.push(key);
      return "/([^/]+)";
    }).replace(/\//g, "\\/");
  ...
  match(method, path2) {
    const [pathname, queryString] = path2.split("?");
...
async function createApp(config) {
  ...
  let db = null;
  if (config.database) {
    const { DatabaseManager: DatabaseManager2 } = await import("./chunk-406d26fb795b694f.js");
    db = DatabaseManager2.init(config.database);
  }
...
//# debugId=E5FF03B2CD9182E164756E2164756E21
