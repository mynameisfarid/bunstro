...
// src/database/ConnectionFactory.ts
...
function format(date, formatStr, options) {
  ...
  const locale = options?.locale ?? defaultOptions6.locale ?? enUS;
  ...
  const weekStartsOn = options?.weekStartsOn ?? options?.locale?.options?.weekStartsOn ?? defaultOptions6.weekStartsOn ?? defaultOptions6.locale?.options?.weekStartsOn ?? 0;
...
class Logger {
  ...
  formatConsoleMessage(entry) {
    ...
    const app = entry.app || this.app ? `${this.dim}[${entry.app || this.app}]${this.reset}` : "";
    const message2 = `${color}${entry.message}${this.reset}`;
    const context = entry.context ? JSON.stringify(entry.context) : "";
    return `${timestamp} ${level} ${app} ${message2} ${context}`;
  }
  ...
  log(level, message2, context) {
    if (!this.shouldLog(level) && level != "app")
      return;
    const entry = {
      level,
      message: message2,
  ...
  debug(message2, context) {
    this.log("debug", message2, context);
  }
  info(message2, context) {
    this.log("info", message2, context);
  }
  warn(message2, context) {
    this.log("warn", message2, context);
  }
  error(message2, context) {
    this.log("error", message2, context);
  }
  fatal(message2, context) {
    this.log("fatal", message2, context);
  }
  child(app) {
...
class DatabaseManager {
  ...
  constructor(config) {
    ...
    this.defaultConnection = config?.default ?? Object.keys(config?.connections ?? {})[0] ?? "default";
  ...
  static getInstance() {
    if (!DatabaseManager.instance) {
      throw new Error("DatabaseManager is not initialized. Call DatabaseManager.init(config) first.");
    }
    return DatabaseManager.instance;
  }
  eagerLoad() {
  ...
  async shutdown() {
    for (const [name, connPromise] of this.connections) {
      try {
        ...
        Logger_default.error(`Failed closing DB: ${name}`, err);
      }
    }
...
var DatabaseManager_default = DatabaseManager.getInstance();
export {
  DatabaseManager_default as default,
  DatabaseManager
};

export { __toESM, __commonJS, Logger, Logger_default, DatabaseManager_default };

//# debugId=406D26FB795B694F64756E2164756E21
