import type { RouteHandler } from './HttpServer';
...
export type RouteMiddleware = (ctx: any, next: () => Promise<any>) => Promise<any>;
...
export class Router {
  ...
  resource(path: string, controller: any): this {
    ...
    if (controller.show) this.get(`${path}/:id`, controller.show.bind(controller));
    if (controller.update) this.put(`${path}/:id`, controller.update.bind(controller));
    if (controller.destroy) this.delete(`${path}/:id`, controller.destroy.bind(controller));
    return this;
  }
