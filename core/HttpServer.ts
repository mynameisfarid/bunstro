...
export class HttpServer {
  ...
  async start(): Promise<void> {
    ...
    logger.log('app', `🚀 Server running on http://${this.config.host}:${this.config.port}`);
  }
  ...
  private async handleRequest(req: Request): Promise<Response> {
    ...
    try {
      ...
      if (this.config.staticPath && path.startsWith(this.config.staticUrl || '/')) {
  ...
  private async serveStatic(path: string): Promise<Response | null> {
    ...
    try {
      ...
      if (process.env.APP_ENV === 'production') {
        this.staticCache.set(path, response.clone());
      }
  ...
  private buildContextFast(
    ...
    return {
      ...
      html(template: string, data: any, status = 200) {
